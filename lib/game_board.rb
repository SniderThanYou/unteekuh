class GameBoard
  ...
  def initialize(region)
    @city_names = self.send("#{region}_city_names")
    @resource_types = self.send("#{region}_resource_types")
    @land_connections = self.send("#{region}_land_connections")
    @water_connections = self.send("#{region}_water_connections")
