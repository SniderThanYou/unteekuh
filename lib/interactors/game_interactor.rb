class GameInteractor
  ...
  def purchase_temples(player_id, city_names)
    ...
    city_names.each do |city_name|
      raise "you do not own #{city_name}" if @game_gateway.owner_of(city_name) != player_id
      raise "#{tech_name} already has a temple" if @game_gateway.has_temple?(city_name)
    end
  ...
  def found_cities(player_id, city_names)
    ...
    city_names.each do |city_name|
      raise 'city already owned' if @game_gateway.owner_of(city_name)
    end
    n = city_names.length
